...
def main():
    ...
    need(digest(archive)=='24b64ccbd1f38cd1ad7921993575d6ebff0638c5d3c694f8e78c8fca3b2be045','Original R6 archive changed')
    with zipfile.ZipFile(archive)as z:
        prefix='C11_Avoidance_Profile_R6_2026-09-08/'
        for member in z.infolist():
            if member.is_dir():continue
            need((ROOT/'frozen'/member.filename[len(prefix):]).read_bytes()==z.read(member),'Frozen R6 file changed')
    ...
    need(report['status']=='PASS_C11_R6_PINNED_LEAN_GATE'and report['local_lean_compilation_completed'],'Missing successful build')
    ...
    for name in ['30_r6_build','31_scopes_and_axioms','32_native_axiom_types']:
    ...
    need(names==report['native_axiom_names']and len(names)==234,'Native dependency set differs')
    ...
    need(len(markers)==len(set(markers))==234 and set(markers)==set(names),'Native type-audit coverage differs')
    need(native.count('NATIVE_BOOL_AXIOM_TYPE:')==234,'Missing native types')
    template=(ROOT/'tools/NativeAxiomAudit.lean.in').read_text().replace('import ShannonBounds.C11AuxiliaryTrade','import ShannonBounds.C11R6')
    expected_source=template.replace('__NATIVE_AXIOM_COMMANDS__','\n'.join('#check_native_bool_axiom '+json.dumps(n)for n in names))
    need(expected_source==(ROOT/'generated_audits/C11R6_NativeAxiomAudit.lean').read_text(),'Generated native type audit differs')
    for d in [186,198,213]:
        ...
        need('error: Type mismatch'in text and f'C11R6D{d}.N + 1'in text,'Wrong cardinality failure')
    ...
    print(json.dumps({'status':'PASS_R6_RELEASE_INTEGRITY_AND_RECORDED_LEAN_CHECKS','files_checked':len(entries),'dimensions':[186,198,213],'scope_aliases':24,'audited_declarations':232,'native_axioms_type_checked':234,'negative_compile_controls':6,'frozen_R6_unchanged':True,'Lean_rerun_by_this_script':False},indent=2))
