...
def main():
    ...
    def save():(out/'R6_BUILD_RESULT.json').write_text(json.dumps(report,indent=2)+'\n')
    ...
    try:
        ...
        run('30_r6_build',['lake','--rehash','--no-ansi','build','ShannonBounds.C11R6'])
        expected=json.loads((WORK/'SOURCE_BINDINGS.json').read_text())['audited_names']
        text=run('31_scopes_and_axioms',['lake','env','lean','-o','C11R6_AxiomAudit.olean','C11R6_AxiomAudit.lean'])
        ...
        template=(WORK/'tools/NativeAxiomAudit.lean.in').read_text().replace('import ShannonBounds.C11AuxiliaryTrade','import ShannonBounds.C11R6')
        native_source=template.replace('__NATIVE_AXIOM_COMMANDS__','\n'.join('#check_native_bool_axiom '+json.dumps(n)for n in native_names))
        (repo/'C11R6_NativeAxiomAudit.lean').write_text(native_source)
        text=run('32_native_axiom_types',['lake','env','lean','-o','C11R6_NativeAxiomAudit.olean','C11R6_NativeAxiomAudit.lean'])
        ...
        for dim in [186,198,213]:
            c=json.loads((WORK/f'frozen/compiled/C11_TYPED_d{dim}.json').read_text())
            card=f'''import ShannonBounds.C11R6D{dim}
open ShannonBounds
example : C11R6D{dim}.code.card = C11R6D{dim}.N + 1 := by
  exact C11R6D{dim}.card_code
'''
            ...
            need('error: Type mismatch'in text and f'C11R6D{dim}.N + 1'in text,'Wrong cardinality rejection reason')
            kind=c['nodes'][-1]['kind'];words=c['tables'][kind]['rows']
            key=json.loads((WORK/'SOURCE_BINDINGS.json').read_text())['cases'][str(dim)]['tables'][kind]
            descriptors=c['letters']
            def apart(u,v):
                for i,j in zip(u,v):
                    aa,mm=descriptors[i]['family'],descriptors[i]['mask'];bb,nn=descriptors[j]['family'],descriptors[j]['mask']
                    if (aa==bb and mm!=nn) or not (mm&(1<<alphabet.index(bb))) or not (nn&(1<<alphabet.index(aa))):return True
                return False
            wordset={tuple(t)for t in words};bad=None
            for i in range(4):
                for a in range(58):
                    candidate=list(words[0]);candidate[i]=a
                    if tuple(candidate)not in wordset and not apart(candidate,words[0]):bad=candidate;break
                if bad is not None:break
            need(bad is not None,'Missing invalid typed-word fixture')
            literal='!['+', '.join(map(str,bad))+']'
            source=f'''import ShannonBounds.C11R6D{dim}
open ShannonBounds
set_option maxHeartbeats 0
set_option maxRecDepth 4000000
set_option synthInstance.maxSize 4000
def badRows : List (Fin 4 → C11R6Base.Ty) := [{literal}] ++ C11R6Tables.rows_{key}
...
'''
        ...
        artifacts=list((repo/'.lake/build/lib/lean/ShannonBounds').glob('C11R6*.olean'))+[repo/'C11R6_AxiomAudit.olean',repo/'C11R6_NativeAxiomAudit.olean']
        need(len(artifacts)==11,'Missing compiled R6 modules/audits')
        report['compiled_artifacts']={str(p.relative_to(repo)):hashlib.sha256(p.read_bytes()).hexdigest()for p in artifacts}
        report['status']='PASS_C11_R6_PINNED_LEAN_GATE'
        report['dimensions']=[186,198,213]
        report['local_lean_compilation_completed']=True
        report['scope_aliases']=24
        report['alphabet_size']=58
        report['general_profile_calculus_formalized']=False
        report['formalization_route']='ORDINARY_58_LETTER_CERTIFICATES_WITH_LITERAL_BASE_PROFILE_PROOF'
        report['historical_root_comparisons']=15
