...
namespace ShannonBounds.C11R9D213
open SimpleGraph C11R6Base C11R6ListChecks C11R6D213

def N : Nat := 15581417495499244082365919592642781336557169190184505729531184988512265817617037617125618829671532871759899754343522415371430987488600093437366857505800109
...
theorem decimal_lower_integer : 5295524149098711 ^ 213 ≤
    N * (1000000000000000 : Nat) ^ 213 := by native_decide

theorem decimal_upper_integer : N * (1000000000000000 : Nat) ^ 213 <
    5295524149098712 ^ 213 := by native_decide

theorem root_bracket :
    (5295524149098711 : ℝ) / 1000000000000000 ≤ (N : ℝ) ^ ((1 : ℝ) / (213 : ℕ)) ∧
    (N : ℝ) ^ ((1 : ℝ) / (213 : ℕ)) < (5295524149098712 : ℝ) / 1000000000000000 := by
  constructor
...
theorem capacity_lower : (5.295524149098711 : ℝ) ≤
    shannonCapacity (SimpleGraph.cycleGraph 11) := by
  rw [show (5.295524149098711 : ℝ) =
    (5295524149098711 : ℝ) / 1000000000000000 by norm_num]
