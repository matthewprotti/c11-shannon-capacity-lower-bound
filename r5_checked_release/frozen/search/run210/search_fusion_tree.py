...
def save(stage):
 ...
 if d not in best_by_dim or N>best_by_dim[d]:
  best_by_dim[d]=N;save_certificate(ROOT/f'fusion_d{d}.json',ns,tt,meta={'parent':inp.name,'records':records})
...
for it in range(24):
 ...
 # Fuse a parent and binary child into one ternary substitution.
 fusion=None;fusion_gain=0
 for t in alln[1:]:
  if t.tab is None or len(t.kids)!=2:continue
  for j,k in enumerate(t.kids):
   if k.tab is None or len(k.kids)!=2:continue
   kids=list(k.kids)+[t.kids[1-j]]
   fake=Node(t.tab,kids);fake.w=t.w;fake.grad=t.grad
   bt,delta=best_table(fake)
   if delta>fusion_gain:fusion=(t,kids,bt);fusion_gain=delta
 if fusion is not None:
  t,kids,bt=fusion;t.kids=kids;t.tab=bt
  new=root.evaluate();assert new==old+fusion_gain and root.d==d
  save(f'fusion_{it}');old=new;alln=[];root.ad(None,alln)
 ...
 if winner is None and not changed:
  # A paired deletion/insertion reallocates one base block at fixed dimension.
  transfer=None;bestTransfer=old
  deletions=[]
  for t in alln[1:]:
   if t.tab is not None and len(t.kids)==2:
    for k in t.kids:
     if k.d==t.d-3:deletions.append((t,k));break
  for t,k in deletions:
   if time.monotonic()-START>LIMIT:break
   original_tab,original_kids=t.tab,t.kids
   t.tab,t.kids=k.tab,k.kids
   low=root.evaluate();lower=[];root.ad(None,lower)
   for target in lower[1:]:
    if max(target.grad)==0:continue
    tags,outs,inds=poly[2]
    vals=np.array([x/max(target.grad)for x in target.grad])[outs]
    vals*=np.array([x/max(target.w)for x in target.w])[inds[:,0]]
    vals*=np.array(BASE,dtype=float)[inds[:,1]]
    scores=np.bincount(tags,weights=vals,minlength=len(library[2]))
    tab=library[2][int(np.argmax(scores))]
    nw=evaluate(tab,[target.w,BASE])
    N=low+sum(g*(b-a)for g,a,b in zip(target.grad,target.w,nw))
    if N>bestTransfer:bestTransfer=N;transfer=(t,k,target,tab)
   t.tab,t.kids=original_tab,original_kids
  root.evaluate()
  if transfer is None:
   if fusion is not None:continue
   break
  t,k,target,tab=transfer
  t.tab,t.kids=k.tab,k.kids
  before=Node(target.tab,target.kids,target.origin)
  target.tab=tab;target.kids=[before,Node()]
  new=root.evaluate();assert new==bestTransfer and root.d==d
  save(f'transfer_{it}')
...
(ROOT/f'fusion_summary_{c["dimension"]}_{CAP}.json').write_text(json.dumps({'elapsed':time.monotonic()-START,'library_counts':{k:len(v)for k,v in library.items()},'records':records},indent=2)+'\n')
