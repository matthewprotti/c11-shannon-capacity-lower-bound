...
def save(stage):
 ...
 if d not in best_by_dim or N>best_by_dim[d]:
  best_by_dim[d]=N;save_certificate(ROOT/f'transfer_d{d}.json',ns,tt,meta={'parent':inp.name,'records':records})
...
(ROOT/f'transfer_summary_{c["dimension"]}_{CAP}.json').write_text(json.dumps({'elapsed':time.monotonic()-START,'library_counts':{k:len(v)for k,v in library.items()},'records':records},indent=2)+'\n')
