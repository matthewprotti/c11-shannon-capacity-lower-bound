...
# Enumerated rowwise hybrids of the transcribed BPZ tables. O is empty.
rowcat=json.loads((ROOT/'row_hybrid_catalog.json').read_text())
for ar,record in rowcat.items():
 for i,combo in enumerate(record['combos']):
  tab={a:[] for a in ABC}
  for a,ix in zip('BNADHV',combo):tab[a]=record['rowsets'][a][ix]
  assert admissible(tab)
  TABLES[f'RowHybrid{ar}_{i}']=tab
...
def save(stage):
 ...
 if d not in best_by_dim or N>best_by_dim[d]:
  best_by_dim[d]=N;save_certificate(ROOT/f'rowhybrid_d{d}.json',ns,tt,meta={'parent':inp.name,'records':records})
...
(ROOT/f'rowhybrid_summary_{c["dimension"]}_{CAP}.json').write_text(json.dumps({'elapsed':time.monotonic()-START,'library_counts':{k:len(v)for k,v in library.items()},'records':records},indent=2)+'\n')
