...
def dimension_ascent():
 global attempts,accepted
 old=root.evaluate();d=root.d;ns=nodes();bestN=old;bestD=d;win=None
 for t in ns[1:]:
  if not max(t.grad):continue
  oldpart=sum(g*w for g,w in zip(t.grad,t.w))
  if d<300:
   leaf=Node();leaf.evaluate();p=polys[2];ws=p.profiles([t,leaf]);scores=ws@np.array([g/max(t.grad)for g in t.grad]);ix=int(np.argmax(scores));tab=p.tabs[ix]
   ww=evaluate(tab,[t.w,BASE]);nn=old+sum(g*w for g,w in zip(t.grad,ww))-oldpart;dd=d+3;attempts+=1
   if nn**bestD>bestN**dd:bestN,bestD=nn,dd;win=('insert',t,tab,None)
  if t.tab is None:continue
  for j,k in enumerate(t.kids):
   if k.tab is not None:continue
   kids=[s for i,s in enumerate(t.kids)if i!=j]
   if len(kids)==1:tab=kids[0].tab;kk=kids[0].kids;ww=kids[0].w
   else:
    p=polys[len(kids)];ws=p.profiles(kids);scores=ws@np.array([g/max(t.grad)for g in t.grad]);ix=int(np.argmax(scores));tab=p.tabs[ix];kk=kids;ww=evaluate(tab,[k.w for k in kids])
   nn=old+sum(g*w for g,w in zip(t.grad,ww))-oldpart;dd=d-3;attempts+=1
   if nn**bestD>bestN**dd:bestN,bestD=nn,dd;win=('delete',t,tab,kk)
 if win is None:return False
 mode,t,tab,kk=win
 if mode=='insert':kk=[Node(t.tab,t.kids),Node()]
 t.tab,t.kids=tab,kk
 if root.evaluate()!=bestN or root.d!=bestD:raise RuntimeError('dimension ascent mismatch')
 accepted+=1;save('root_improving_'+mode);return True
save('start')
try:
 for r in range(args.rounds):
  if time.monotonic()-START>args.seconds:break
  ss=sweep()
  if ss:save(f'dimension_table_sweep_{r}_{ss}')
  moved=dimension_ascent()
  win,best=restructure_round()
  if win:
   t,tab,kids,meta=win;t.tab,t.kids=tab,kids
   if root.evaluate()!=best:raise RuntimeError('growth reassembly mismatch')
   accepted+=1;save(f'dimension_reassembly_{r}_{meta}')
  elif not moved and not ss:break
