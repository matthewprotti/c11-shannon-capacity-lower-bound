...
for j,k in enumerate(root.kids):
 if k.tab is not None and len(k.kids)==2:
  old=root.evaluate(); words=[]
  for word in root.tab:
   for block in k.tab[word[j]]:words.append(word[:j]+block+word[j+1:])
  if not admissible(words):raise RuntimeError('expanded terminal inadmissible')
  root.tab=sorted(words);root.kids=root.kids[:j]+k.kids+root.kids[j+1:]
  if root.evaluate()!=old:raise RuntimeError('expanded terminal count mismatch')
  save('terminal_expanded_to_four_blocks')
  break
if len(root.kids)==4:
 for r in range(10):
  if time.monotonic()-START>args.seconds:break
  if not trade_sweep():break
save('stopped')
